...
namespace GenericEventBus
{
    ...
    {
        public delegate void TargetedEventHandler<TEvent>(
            ref TEvent eventData,
            TObject target,
            TObject source
        );

        private static readonly EqualityComparer<TObject> ObjectComparer =
            EqualityComparer<TObject>.Default;
        ...
        public bool Raise<TEvent>(TEvent @event, TObject target, TObject source)
        ...
        public bool RaiseImmediately<TEvent>(TEvent @event, TObject target, TObject source)
            where TEvent : TBaseEvent
        {
            return RaiseImmediately(ref @event, target, source);
        }
        ...
        public void UnsubscribeFrom<TEvent>(TargetedEventHandler<TEvent> handler)
            where TEvent : TBaseEvent
        {
            var listeners = TargetedEventListeners<TEvent>.Get(this);
            listeners.RemoveListener(handler);
        ...
        ///                        If multiple listeners have the same priority, they will be invoked in the order they subscribed.</param>
        /// <typeparam name="TEvent">The event type to subscribe to.</typeparam>
        public void SubscribeToTarget<TEvent>(
            TObject target,
            TargetedEventHandler<TEvent> handler,
            float priority = 0
        )
            where TEvent : TBaseEvent
        {
        ...
        /// <typeparam name="TEvent">The event type to unsubscribe from.</typeparam>
        public void UnsubscribeFromTarget<TEvent>(
            TObject target,
            TargetedEventHandler<TEvent> handler
        )
            where TEvent : TBaseEvent
        ...
        ///                        If multiple listeners have the same priority, they will be invoked in the order they subscribed.</param>
        /// <typeparam name="TEvent">The event type to subscribe to.</typeparam>
        public void SubscribeToSource<TEvent>(
            TObject source,
            TargetedEventHandler<TEvent> handler,
            float priority = 0
        )
        ...
        /// <typeparam name="TEvent">The event type to unsubscribe from.</typeparam>
        public void UnsubscribeFromSource<TEvent>(
            TObject source,
            TargetedEventHandler<TEvent> handler
        )
            where TEvent : TBaseEvent
        ...
        private class TargetedEventListeners<TEvent>
            where TEvent : TBaseEvent
        {
            private static readonly Dictionary<
                GenericEventBus<TBaseEvent, TObject>,
                TargetedEventListeners<TEvent>
            > Listeners =
                new Dictionary<
                    GenericEventBus<TBaseEvent, TObject>,
                    TargetedEventListeners<TEvent>
            ...
            private static readonly Dictionary<
                EventHandler<TEvent>,
                TargetedEventHandler<TEvent>
            > ConvertedEventHandlers =
                new Dictionary<EventHandler<TEvent>, TargetedEventHandler<TEvent>>();

            private static readonly ObjectPool<Enumerator> EnumeratorPool =
                new ObjectPool<Enumerator>();

            private static readonly ObjectPool<DerivedQueuedEvent> QueuedEventPool =
                new ObjectPool<DerivedQueuedEvent>();

            private static readonly ObjectPool<List<Listener>> ListenerListPool =
                new ObjectPool<List<Listener>>();
            ...
            public static TargetedEventListeners<TEvent> Get(
                GenericEventBus<TBaseEvent, TObject> eventBus
            )
            {
            ...
            public void RemoveListener(TargetedEventHandler<TEvent> handler)
            {
                ...
                {
                    if (!Equals(_sortedListeners[i].Handler, handler))
                        continue;
            ...
            public void AddListener(EventHandler<TEvent> handler, float priority)
            {
                ...
                {
                    convertedHandler = (ref TEvent data, TObject target, TObject source) =>
                        handler(ref data);
            ...
            )
            {
                ...
                foreach (var enumerator in _activeEnumerators)
                {
                    if (!ObjectComparer.Equals(target, enumerator.Target))
                        continue;
            ...
            public void RemoveTargetListener(TObject target, TargetedEventHandler<TEvent> handler)
            {
                ...
                {
                    if (!Equals(listeners[i].Handler, handler))
                        continue;
            ...
            )
            {
                ...
                foreach (var enumerator in _activeEnumerators)
                {
                    if (!ObjectComparer.Equals(source, enumerator.Source))
                        continue;
            ...
            public IEnumerable<TargetedEventHandler<TEvent>> GetListeners(
                TObject target,
                TObject source
            )
            {
            ...
            private class Enumerator
                : IEnumerator<TargetedEventHandler<TEvent>>,
