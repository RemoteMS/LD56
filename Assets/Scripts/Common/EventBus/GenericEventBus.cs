...
namespace GenericEventBus
{
    ...
    public class GenericEventBus<TBaseEvent> : IService, IDisposable
    {
        ...
        public bool CurrentEventIsConsumed =>
            _currentRaiseRecursionDepth > 0
            && _raiseRecursionsConsumed.Contains(_currentRaiseRecursionDepth);
        ...
        public delegate void EventHandler<TEvent>(ref TEvent eventData)
        ...
        public bool RaiseImmediately<TEvent>(TEvent @event)
            where TEvent : TBaseEvent
        {
        ...
        /// <typeparam name="TEvent">The type of event to raise.</typeparam>
        /// <returns>Returns true if the event was consumed with <see cref="ConsumeCurrentEvent"/>.</returns>
        public virtual bool RaiseImmediately<TEvent>(ref TEvent @event)
            where TEvent : TBaseEvent
        {
        ...
        public virtual bool Raise<TEvent>(in TEvent @event)
            where TEvent : TBaseEvent
        {
        ...
        public virtual void UnsubscribeFrom<TEvent>(EventHandler<TEvent> handler)
            where TEvent : TBaseEvent
        {
            var listeners = EventListeners<TEvent>.GetListeners(this);
        ...
        public void ClearListeners<TEvent>()
            where TEvent : TBaseEvent
        {
            if (IsEventBeingRaised)
            {
                throw new InvalidOperationException(
                    "Not allowed to clear listeners while an event is being raised."
                );
            }
        ...
        protected virtual void ClearAllListeners<TEvent>()
            where TEvent : TBaseEvent
        {
            var listeners = EventListeners<TEvent>.GetListeners(this);
            listeners.Clear();
        ...
        private sealed class EventListeners<TEvent> : IEnumerable<EventHandler<TEvent>>
            where TEvent : TBaseEvent
        {
            private static readonly Dictionary<
                GenericEventBus<TBaseEvent>,
                EventListeners<TEvent>
            ...
            private class Enumerator : IEnumerator<EventHandler<TEvent>>
            {
                ...
                {
                    if (Index >= Owner._sortedListeners.Count)
                        return false;
